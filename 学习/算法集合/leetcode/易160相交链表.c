...
/*
思路：
   该题的题目描述并不严谨，经过搜索发现，该题的相交指的只是例子里那种形式，即只存在两个链表合为一条链表这种情况，而不存在从一条链表出
现两个分支这种情况。在此条件下，首先判断长度，二者长度相等则依次比较两链表节点地址是否相等即可，若长度不等，将长链表指针从表头处移动二
者差值个长度，再重复相等情况的操作就 OK 了。
*/