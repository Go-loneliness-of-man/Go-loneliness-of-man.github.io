...
int main()
{
	int i, j, flag, x, y, z;			char a[101], *b, P, T;
	...
	{
		gets(a);															//获取一个字符串
		for (b = a, flag = x = y = z = P = T = 0; *b != '\0'; b++)			//判断一个字符串
		{
		...
		else if (flag == 1 || P != 1 || T != 1)		printf("NO\n");
		//有其它字符或没有 A、P 或 T 的数量不等于 1，不通过
		else	printf("YES\n");
	}
	return 0;
}
...
注意看 aPbTc 和 aPbATca 这两个式子，初始式子都是 xPATx 形式，由前者到后者是在 P、T 中间和 T 后加 A，并且方式是在 T 后加上 a 个
A ，再在 P、T 中间加上一个 A，又因为初始式子恒有 a = c，所以往后的式子将始终满足 a * b = c，换句话说，P 前面的 A 乘 P、T 之间的
A 的数量要等于 T 后 A 的数量。
